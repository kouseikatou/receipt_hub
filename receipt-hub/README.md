...
4. [環境構築](#環境構築)
   ...
   - [Step 5: セットアップスクリプトを実行](#step-5-セットアップスクリプトを実行)
5. [使い方](#使い方)
...
4. URLをコピーしておく（次のセットアップスクリプトで使用）

---

### Step 5: セットアップスクリプトを実行

以下のコマンド1つで、スプレッドシートURLの登録・ローカルフォルダの作成・接続確認をまとめて行います。
...
実行すると以下が自動で完了します：
- OAuth認証の確認
- スプレッドシートURLの入力 → `~/.receipt-hub/config.json` に保存
- 接続テスト（URLが正しいか確認）
